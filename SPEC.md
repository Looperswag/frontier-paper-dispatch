...
## 目标
自动采集 arxiv / huggingface / github / 大厂官方 blog（X best-effort，FB 可选），用 DeepSeek 依据「我的画像」排名 Top5、生成概要与「对我的影响」，每晚十点推送到微信（Server酱）；并提供复古牛皮纸阅读前台（批注 + 右栏 chatbot + md/pdf/word 导出）。
...
## 阶段
- **Phase 1（本仓库当前范围）**：采集 + 排名 + 总结/影响 + 每日 digest + Server酱 微信推送 + launchd。
...
3. 配好 `.env` 后 `npm run ingest` 写入 Supabase；`npm run ingest:send` 微信收到 Top5。
